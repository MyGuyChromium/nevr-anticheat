...
var detectorSpecs = map[string]DetectorSpec{
	...
	"STATE_008": {ID: "STATE_008", Name: "Autopocket Catch Review", Category: "state",
		Params: []ParamSpec{
			...
			f("min_correction_angle_deg", 4.0, "deg", "minimum sustained heading correction toward the eventual catcher (clamped to [1, 10])"),
			f("max_turn_angle_deg", 20.0, "deg", "larger single-sample turns are ambiguous (clamped to [10, 30])"),
