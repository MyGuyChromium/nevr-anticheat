...
var detectorSpecs = map[string]DetectorSpec{
	...
	"STATE_008": {ID: "STATE_008", Name: "Pre-catch Trajectory Review", Category: "state",
		...
		Params: []ParamSpec{
			i("baseline_samples", 4, "samples", "free-flight samples for the provisional trajectory baseline (clamped to [4, 12])"),
			i("min_correction_samples", 2, "samples", "qualifying approach corrections before a catch review (clamped to [2, 8])"),
			f("baseline_duration_s", 0.20, "s", "minimum elapsed stable reference duration, in addition to the sample floor (clamped to [0.10, 1])"),
			f("min_correction_duration_s", 0.12, "s", "minimum sustained corroborated turning duration, in addition to the sample floor (clamped to [0.06, 1])"),
			f("min_turn_rate_deg_s", 60.0, "deg/s", "minimum time-normalized correction rate, not a legal-physics limit (clamped to [15, 180])"),
			f("max_turn_rate_deg_s", 300.0, "deg/s", "larger time-normalized turns are ambiguous (clamped to [180, 720])"),
			f("max_sample_gap_s", 0.12, "s", "largest eligible adjacent sample gap (clamped to [0.02, 0.20])"),
			f("max_window_s", 1.5, "s", "maximum pre-catch trajectory window (clamped to [0.25, 2])"),
			f("max_step_error_m", 0.20, "m", "maximum displacement/velocity disagreement (clamped to [0.01, 0.5])"),
			f("min_lateral_deviation_m", 0.30, "m", "minimum deviation from the baseline trajectory (clamped to [0.1, 3])"),
			f("contact_margin_m", 0.65, "m", "possible-contact exclusion, not a verified game catch radius (clamped to [0.5, 2])"),
			f("contact_accel_allowance_mps2", 30.0, "m/s^2", "provisional interpolation envelope allowance; expands contact margin with sample duration, not a proven acceleration bound (clamped to [0, 200])"),
			f("min_miss_improvement_m", 0.50, "m", "minimum projected miss-distance reduction toward the catcher (clamped to [0.25, 3])"),
			f("max_catch_approach_m", 1.5, "m", "maximum last-free-sample hand distance for catch association (clamped to [0.5, 2])"),
			f("regrab_grace_s", 0.35, "s", "exclude rapid regrabs after an observed release (clamped to [0.25, 1])"),
		}},
