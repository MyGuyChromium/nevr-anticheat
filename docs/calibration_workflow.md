...
5. Use **Ground-truth window** for exploratory annotations, including missed detections. For scored-review eligibility, use the [Evidence review workspace](blind_review_workflow.md): attach the actual independent artifact, bind its SHA-256 to an exact player/window and candidate, collect two agreeing immutable pre-reveal ballots from distinct reviewers, then explicitly reveal. URL strings, free-text references and legacy reviewer attestations alone no longer pass the evidence gate. Use `controlled_reproduction`, `synchronized_video`, or `authoritative_telemetry`; an allegation, player-level admission, or NEVR's own measured speed is not independent window evidence.
...
8. Export the portable evidence library regularly, and keep a full database backup. The portable JSON preserves annotations but does not export attached evidence bytes or transfer independent-review provenance. Immutable review sessions, ballots, and evidence attachments are included in the full database backup. Restore the backup with the app's verified recovery workflow.
...
- Every decisive sample: matching current app/build/behavior provenance, good or excellent ungated telemetry, no isolation conflict, and a verified hash-bound session with two agreeing immutable pre-reveal ballots. Direct event labels and legacy textual attestations alone cannot pass this evidence gate. Count floors alone are not enough to satisfy the confidence bounds.
- Connected groups: at least 10 overall and five in each evaluation split, with at least three positive and three negative groups. No group may supply more than 25% of overall decisive opportunities or 40% of a split. Required ping/capture/legal-contact strata each need at least 20 legitimate opportunities across three groups. These are conservative diversity policies, not measured validity; see the [review workflow](blind_review_workflow.md) for the exact strata.
