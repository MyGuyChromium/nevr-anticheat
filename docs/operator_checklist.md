...
- [ ] With `configs/shadow_deploy.toml`, the startup table command `grep '"msg":"effective detector"' server.log | grep -v '"mode":"shadow"'` prints nothing and `grep -c '"msg":"effective detector"' server.log` is 30.
