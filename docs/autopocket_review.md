...
Desktop 0.13.0 / STATE_008 0.3.0 names this check **Pre-catch Trajectory Review**. Earlier builds used the provisional “Autopocket Catch Review” label; no verified implementation evidence establishes that community nickname's mechanism. This check provides a conservative trajectory observation, not a verified autopocket verdict. It finds a steady free disc making sustained, position-and-velocity-supported corrections toward a moving receiver's hand, followed by attachment confirmed in two samples. It cannot establish input automation. See the [mechanics capability audit](mechanics_capability_audit.md) for distinct disc-grab reach (mags), shot targeting, release physics and settings checks.
...
1. Require a complete fresh sampled roster (at most 16 players), finite disc data, known nonconflicting hand attachment and bound source context, an explicitly present bounce counter, and tracked head/body/both hands for every player. Missing is unknown, including a missing bounce counter; it is never silently treated as zero/no-contact. Legacy possession flags alone cannot establish the attachment. Spectators are excluded. A rejected player sample leaves the roster incomplete.
