# Investigation and reliability suite (desktop v0.5)
...
7. **Precision/recall dashboard** — event-label precision is exact; recall is explicitly limited to the match-level fraction of confirmed-cheat replays with any signal.
8. **Holdout replay library** — stable hash-based 60/20/20 training, validation, and holdout partitions prevent replays from silently changing groups.
...
21. **Portable evidence library** — JSON export/import carries human match labels, direct event reviews, notes/bookmarks, and saved filters; raw telemetry is excluded.
...
24. **Experiment matrix** — 1–12 numeric candidates run in memory on stored telemetry and return event count/runtime without persistence.
