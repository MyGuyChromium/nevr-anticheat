...
5. Label individual observations in the desktop app (**Correct**, **False positive**, or **Unsure**, plus an optional note) and/or decide review cases with `verdict`. `./nevr-ac --config calib.toml calibration-report` combines those direct labels with per-case feedback into per-detector confirmed / false-positive counts and precision. Direct labels are retained even when derived events are replaced by re-analysis. Promote a detector to `mode = "review"` only when its precision is acceptable over a meaningful labeled sample; demote (`mode = "shadow"`) at the first confirmed false positive.
