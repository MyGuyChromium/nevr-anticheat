...
**Bottom line: 0 of 30 detectors are validated on labelled real Echo VR telemetry.** Every threshold still needs calibration. Both shipped configurations are all-shadow so false-positive observations cannot affect scores or cases.
...
| Field | Issue | Status |
|-------|-------|--------|
| Rotation / HandRotation | API gives `{forward, left, up}` direction vectors, not quaternions | **DONE**: `model.QuatFromDirectionVectorsChecked` normalises forward, Gram-Schmidts up and rebuilds left = up × forward, so the result is always a proper rotation. It measures the handedness of the input (`BasisProper` vs `BasisReflected`, counted in `MapperStats.BasesProper/BasesReflected`) and handles both conventions exactly. **Confirmed on real data (2026-09-03)**: three real recordings decode as 100 % proper (det +1) bases; the synthetic fixtures were the reflected ones. The bridge still logs `basis_reflected` once if a source ever sends the other convention. A degenerate basis (zero or collinear vectors) yields the **zero quaternion** (tracking loss), never a silent identity. |
...
|----------|--------|-------------------|----------------|-------------|
| THROW_001 | **PHYSICS_GROUNDED** | Yes | Not until replay telemetry is validated | Historical status label, not validation or promotion. The owner retained 18.9 m/s as a configured review threshold, not a verified engine ceiling. First-free speed, three-sample free-flight review and a bound local-client `last_throw` report stay separate. A corroborated sampled-speed observation is not proof of contact-free launch or cheating; incomplete windows and isolated above-threshold spikes remain uncorroborated, zero-weight observations. Player-relative speed remains descriptive context, never the primary comparison. See [default settings and evidence](default_game_config_reference.md). |
...
| BIO_001 | **UNVERIFIED** | Yes (rotation-format probe) | No | **Saturation:** wrist rate is bounded by π/dt. At a steady 0.067 s interval the 50 rad/s threshold cannot be exceeded (`Bio001.Reachable(0.067) == false`). A nominally 15 Hz recording can contain shorter intervals; reachability must be checked per actual sample, not inferred from its average rate. Per-player coverage now records necessary-input samples, without claiming aliased motion is recoverable or that contact/tracking artifacts are cheating. |
...
| MOV_006 | **PHYSICS_GROUNDED** | Yes | No; calibrate before promotion | Reconstructs the arena-space anchor from Echo's raw `player.velocity` and subtracts it from tracked head/body motion, so normal locomotion, stacking and geometry boosts cancel while physical room-scale translation remains. Distance, observed-pose-motion, real-time duration, hand/head coherence and ping gates reduce noise; the pose-motion gate specifically rejects frozen remote snapshots carrying stale non-zero game velocity. No feet/guardian telemetry exists to rule out every legal lean or lunge. Ships shadow-only. |
...
The desktop regression lab reduces repeat regressions: direct Correct / False positive event labels are matched to current detector output by match, player, detector and a small frame tolerance. Ground-truth behavior windows additionally measure missed positive opportunities and correctly silent legitimate opportunities, producing TP/FP/FN/TN, confidence intervals, and player-grouped training/validation/holdout results. Promotion requires current-build/current-behavior provenance and can enable only one scored human-review detector; automatic enforcement remains disabled. This is still only as strong as the owner's reviewed replay library and cannot replace server-side ground truth.
...
These historical counts are not accuracy measurements. In particular, an observation above the configured 18.9 m/s threshold is not independently established as an impossible throw, even if its numerical value is finite. Release/contact timing, first-free transients and the rule's build applicability remain unresolved. The newer sampled-speed evidence review preserves those limitations; a broader independently labelled sample is required.
...
2. **THROW_001 auto-enforcement** stays off (`auto_enforce = false`). Version 2.0.0 never stamps `AutoEnforce`, even if an older configuration requests it. Repeated client-reported speeds are not authority to punish; the backend review-only policy remains in force.
