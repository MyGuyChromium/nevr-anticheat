...
| MOV_006 | **PHYSICS_GROUNDED** | Yes | No; calibrate before promotion | Reconstructs the arena-space anchor from Echo's raw `player.velocity` and subtracts it from tracked head/body motion, so normal locomotion, stacking and geometry boosts cancel while physical room-scale translation remains. Stricter distance, real-time duration, hand/head coherence and ping gates reduce noise, but no feet/guardian telemetry exists to rule out every legal lean or lunge. Ships shadow-only. |
