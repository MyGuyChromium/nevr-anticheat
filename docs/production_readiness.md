...
|----------|--------|-------------------|----------------|-------------|
| THROW_001 | **PHYSICS_GROUNDED** | Yes | Not until replay telemetry is validated | Uses the configured 18.9 m/s reference cap. Sampled speed comes from `disc.velocity`; a matched local-player engine `last_throw` record can supply a higher independent scalar. Neither source establishes remote release/contact timing or a universal reference-frame rule. Evidence includes player speed, aligned movement, and player-relative sampled speed. The direction projection uses the sampled vector magnitude, not the engine scalar. Releases > 2× cap are low-confidence artifacts. It operates on 15 Hz input and repeated below-cap throws do not produce an event. See [measurement verification](throw_measurement_verification.md) for what runtime source does and does not establish. |
