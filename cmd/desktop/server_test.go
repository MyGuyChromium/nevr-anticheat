...
func TestDesktop_IndexIncludesFullMatchReport(t *testing.T) {
	...
	resp.Body.Close()
	page := string(raw)
	for _, marker := range []string{"Analyze a replay.", "Review the evidence.", "LOCAL ENGINE", "offline-banner", "Stored replays refresh automatically", "Full match report", "Player statistics", "Scoring timeline", "Throw log", "cap-breach", "Download JSON", "Export player CSV", "Open clip", "Open replay viewer", "Filter by detector", "shadow review", "Neither status is a verified cheating verdict", "data-replay-frame", "data-throw-scroll", "Spark replay", "Diagnostic ZIP", "Physics inspector", "Telemetry &amp; schema health", "Calibration library", "Export hashed calibration packet", "Archive + remove active raw", "Detector observations", "scan a folder", "Stop queue", "Detector verdict", "History filters", "Regression &amp; threshold lab", "Threshold sandbox", "Before / after", "Player history", "Replay watch folder", "Crash recovery", "Windows updates", "Install update", "Support bundle", "Health &amp; maintenance", "Back up database"} {
