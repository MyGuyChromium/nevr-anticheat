...
Desktop 0.13.0 added [grab, release and settings hardening](docs/mechanics_capability_audit.md): explicit hand attachments, confirmed sampled releases, rotation validity, correctly attributed local diagnostics, and bounded mechanics assessments with Spark/physics links. The later [evidence-health audit](docs/evidence-health-audit.md) corrects the earlier 19/4.7 interpretation: the cited patch's **4.7 m/s** concerns chain carrying the disc, not a verified minimum player speed for every 19 m/s throw; its **0.25** regrab value is seconds, not a grab radius. The owner-supplied numeric settings and separate 18.9 m/s cap remain unchanged, but those source claims do not validate a universal release formula or the proposed 0.25 m grab rule. Accuracy alone is not a cheat finding. Re-import original replays to recover source fields that older normalized caches cannot provide.
